...
export async function GET(
  ...
  try {
    ...
    const response = await coreFetch(coreNode, '/blueprints', { method: 'GET' });

    if (!response.ok) {
      return NextResponse.json({ error: 'Failed to fetch blueprints from core' }, { status: response.status });
    }
