...
export async function POST(
  ...
  try {
    ...
    const url = `${protocol}://${nodeRow.ip_address}:${nodeRow.port}/system/reboot`;
    ...
    if (!response.ok) return NextResponse.json({ error: 'Failed to reboot core' }, { status: response.status });
