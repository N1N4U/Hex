...
export async function POST(
  ...
  try {
    ...
    const url = `${protocol}://${nodeRow.ip_address}:${nodeRow.port}/system/shutdown`;
    ...
    if (!response.ok) return NextResponse.json({ error: 'Failed to shutdown core' }, { status: response.status });
