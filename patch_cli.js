const fs = require('fs');
let c = fs.readFileSync('cli/hex.sh', 'utf8');
const oldBlock = `  update)
    if [ "$EUID" -ne 0 ]; then
    ...
    if wget -q -O /var/lib/hex/core/blueprints/update.py https://raw.githubusercontent.com/N1N4U/Hex/main/others/blueprints/generate.py; then
        ...
        echo "Failed to update blueprints."
    fi
...
c = c.replace(oldBlock, newUpdate);
fs.writeFileSync('cli/hex.sh', c);
